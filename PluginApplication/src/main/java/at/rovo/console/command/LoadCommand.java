...
/**
 * <code>LoadCommand</code> invokes loading of a .jar-file containing a {@link IPlugin} implementation.
 * <p/>
 * Loading can be invoked by either providing the absolute path to the plug-in containing JAR file or the plug-in
 * directory specified in the application is taken on prepending './' or '.\' to the JAR file located in the plug-in
 * directory.
 * <p/>
 * If no command was specified 'load' is taken as default command.
 * <p/>
 * Examples:
 * <code>
 *     <ul>
 ...
 *     </ul>
 * </code>
 * In the above examples the jar file-types ('.jar') can be omitted.</p> <p>Note that multiple instances with different
 * names can be created to register the same action to different commands.</p>
 *
...
{
    /**
     * Creates a new instance of this class and sets its command the {@link Console} will listen to to 'load'.
     *
     * @param manager
    ...
    public void execute(String[] token)
    {
        ...
        if (token.length != 2 && token[1].startsWith("'"))
        {
            ...
            {
                System.out.println(
                        "Invalid call! Usage: 'load plugin' where plugin is the path and the name of the plugin to load");
            }
        }
